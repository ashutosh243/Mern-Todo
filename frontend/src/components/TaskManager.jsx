...
export const TaskManager = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing) {
        await axios.patch(`${config.backendEndpoint}/${taskData._id}`, {
        ...
        await axios.post(`${config.backendEndpoint}`, formData);
      }
      const response = await axios.get(`${config.backendEndpoint}`);
      setTasks(response.data.data);
      handleClose();
  ...
  const handleMarkAsDone = async (taskId) => {
    try {
      await axios.patch(`${config.backendEndpoint}/${taskId}`, {
        status: "DONE",
      });
      const response = await axios.get(`${config.backendEndpoint}`);
      setTasks(response.data.data);
  ...
  const handleDelete = async (taskId) => {
    if (window.confirm("Are you sure you want to delete this task?")) {
      try {
        await axios.delete(`${config.backendEndpoint}/${taskId}`);
        const response = await axios.get(`${config.backendEndpoint}`);
        setTasks(response.data.data);
      } catch (err) {
